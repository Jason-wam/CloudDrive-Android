...
public class AndroidMediaPlayer extends AbstractPlayer implements MediaPlayer.OnErrorListener,
    ...
    public void initPlayer() {
        ...
        setOptions();
        mMediaPlayer.setWakeMode(mAppContext, PowerManager.PARTIAL_WAKE_LOCK);
        ...
        mMediaPlayer.setOnTimedTextListener(new MediaPlayer.OnTimedTextListener() {
            ...
            public void onTimedText(MediaPlayer mp, TimedText text) {
                xyz.doikki.videoplayer.model.TimedText timedText = new xyz.doikki.videoplayer.model.TimedText();
                timedText.text = text.getText();
    ...
    public List<Track> getSubtitles() {
        ...
        MediaPlayer.TrackInfo[] trackInfoList = mMediaPlayer.getTrackInfo();
        for (int i = 0; i < trackInfoList.length; i++) {
            MediaPlayer.TrackInfo trackInfo = trackInfoList[i];
            if (trackInfo.getTrackType() == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT) {
                trackList.add(new Track(i, trackInfo.getLanguage()));
            }
        }
        return trackList;
    }
    ...
    public int getSelectedSubtitleIndex() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return mMediaPlayer.getSelectedTrack(MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT);
        } else {
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        for (int i = 0; i < trackInfoList.length; i++) {
            MediaPlayer.TrackInfo trackInfo = trackInfoList[i];
            if (trackInfo.getTrackType() == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT) {
                Log.i("AndroidMediaPlayer", "subtitle = " + trackInfo.getLanguage());
            }
        }
    }
