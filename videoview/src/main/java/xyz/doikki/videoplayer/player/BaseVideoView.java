...
public class BaseVideoView<P extends AbstractPlayer> extends FrameLayout
    ...
    public List<Track> getSubtitles() {
        ...
        return mMediaPlayer.getSubtitles();
    }
    ...
    public void selectSubtitle(Track track) {
        try {
            if (mMediaPlayer != null) {
                mMediaPlayer.selectSubtitle(track);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int getSelectedSubtitleIndex() {
        ...
        try {
            selectedSubtitleIndex = mMediaPlayer.getSelectedSubtitleIndex();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void onTimedText(TimedText timedText) {
        if (mVideoController != null) {
            mVideoController.onTimedText(timedText);
        }
        for (OnTimedTextListener onTimedTextListener : onTimedTextListeners) {
            onTimedTextListener.onTimedText(timedText);
        }
    }

    private ArrayList<OnTimedTextListener> onTimedTextListeners = new ArrayList<>();

    public void addOnTimedTextListener(OnTimedTextListener listener) {
        onTimedTextListeners.add(listener);
    }
