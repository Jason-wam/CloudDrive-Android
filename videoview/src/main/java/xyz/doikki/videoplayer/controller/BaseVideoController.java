    ...
    protected Runnable mShowProgress = new Runnable() {
        ...
        public void run() {
            ...
            if (controlWrapper.isPlaying()) {
                postDelayed(this, (long) ((1000 - pos % 1000) / controlWrapper.getSpeed()));
            } else {
                mIsStartProgress = false;
            }
        }
...
//        }
     ...
     * 全屏模式下根据视频宽高自动切换横竖屏
     */
    public void setFullScreenDirectionByVideoSize() {
...
//            return;
//        }
        int[] size = controlWrapper.getVideoSize();
        int width = size[0];
        int height = size[1];
        if (width == height) {
            return;
        ...
        if (width > height) {
            if (PlayerUtils.isNotInLandscape(mActivity)) {
                mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
            }
        } else {
            if (!PlayerUtils.isNotInLandscape(mActivity)) {
...
//        }
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        if (controlWrapper.isPlaying() && (mEnableOrientation || controlWrapper.isFullScreen())) {
            if (hasWindowFocus) {
    ...
    public void onTimedText(TimedText timedText) {
        for (Map.Entry<IControlComponent, Boolean> next : mControlComponents.entrySet()) {
            IControlComponent component = next.getKey();
            component.onTimedText(timedText);
        }
    }
