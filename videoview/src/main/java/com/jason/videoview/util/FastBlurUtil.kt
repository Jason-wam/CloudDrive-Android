...
object FastBlurUtil {
    private fun Bitmap.scale(scale: Float): Bitmap {
        val input = this.copy(Bitmap.Config.ARGB_8888, true)
        val matrix = Matrix()
        matrix.postScale(scale, scale)
        return Bitmap.createBitmap(input, 0, 0, input.width, input.height, matrix, true)
    }

    fun doBlur(sentBitmap: Bitmap, radius: Int, scale: Float, canReuseInBitmap: Boolean): Bitmap {
        val bitmap: Bitmap = if (canReuseInBitmap) {
            sentBitmap.scale(scale)
        } else {
            sentBitmap.scale(scale).copy(sentBitmap.config, true)
        }
        if (radius < 1) {
            return sentBitmap
        }
